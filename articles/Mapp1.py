...
class Ui_MainWindow(object):
	...
	def table_update(self):
		...
		for i in range(0,100):
			self.tableWidget.setItem(i,0, QtWidgets.QTableWidgetItem(df2.iloc[i][0]))
	...
	def tabledata(self):
		...
		b=[]
		for i in range(0,100):
			self.tableWidget.setItem(i,0, QtWidgets.QTableWidgetItem(df.iloc[i][0]))
			#header.setSectionResizeMode(i, QtWidgets.QHeaderView.ResizeToContents)
			b.append(self.make_button(i))
	...
	def make_button(self,i):
		...
		btn.clicked.connect(lambda j=i: self.tag_update(df.iloc[int(i)][1]))
